...
void inicializarFechado(HashFechado *tabela, int tamanho) {
    ...
    tabela->posicoes = (Node **)malloc(tamanho * sizeof(Node *));
    for (i = 0; i < tamanho; i++) {
...
void inserirFechado(HashFechado *tabela, Filme filme) {
    ...
    Node *atual = tabela->posicoes[indice];
    Node *novo;
    ...
    novo->filme = filme;
    novo->proximo = tabela->posicoes[indice];
    tabela->posicoes[indice] = novo;
    ...
    while (atual != NULL) {
        if (atual->filme.id == id) {
            if (anterior == NULL) { // Primeiro nó
                tabela->posicoes[indice] = atual->proximo;
            } else {
...
void listarFechado(HashFechado *tabela) {
    ...
    for (i = 0; i < tabela->tamanho; i++) {
        printf("Posicao %d:\n", i); // Renomeado para "Posicao"
        atual = tabela->posicoes[i];
        if (atual == NULL) {
            printf("  Vazio\n");
        }
        while (atual != NULL) {
